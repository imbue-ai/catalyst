...
def list_templates():
    ...
    for item in os.listdir(templates_dir):
        if not item.startswith(".") and os.path.isdir(
...
def create_task(request: str = Form(...), file: Optional[UploadFile] = File(None)):
    ...
    summary_candidate = (
        inputs.get("phenomenon") or inputs.get("idea") or inputs.get("goal") or ""
    )
...
def inject_disclaimer(content: str) -> str:
    disclaimer = "*This document has been generated by the Imbue Catalyst AI Scientist tool. It may contain mistakes, misrepresentations, or lack important citations. Please verify all claims.*"
    lines = content.split("\n")
