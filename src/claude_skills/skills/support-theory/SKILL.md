...
4. **Plan**: For each statement in `statements.md`, decide on a support strategy (proof, experiment, literature citation, or combination). Prioritize the theory's most central / most uncertain statements.
5. **Gather support**: Work through the statements. For each:
...
6. **Iterate**: If an experiment or derivation comes back ambiguous or negative, do not abandon the statement immediately — try a refined experimental setup or an alternative proof strategy. Only record a statement as unsupported after genuinely exhausting reasonable approaches.
7. **Reporting**: Write the final supported theory to `<OUTPUT_DIR>/theory.md` (this exact filename is required). Add plots and figures generated by your experiments; you may also generate additional illustrative plots with Python scripts as needed.
8. **Store results**: Persist your output and report the theory ID:
   ```bash
   ...
   ```
   Note down the returned theory ID (e.g. `T_20260421_150000_x1y2z3`) as the final result of this skill.
