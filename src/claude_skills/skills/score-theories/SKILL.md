...
14. **Overall Theory Score**: Combine the scores for each theory into a final score using the formula: `Overall Score = (0.7 * Prediction Accuracy Score + 0.3 * Soundness Score) * (0.4 + 0.3 * Predictive Power Score + 0.3 * Prediction Coverage Score)`.
15. **Final Output**: Report the list of all theory IDs along with their final scores, sorted from highest to lowest score. Also include a breakdown of the prediction accuracy, prediction coverage, soundness and predictive power scores for each theory.
