...
4. **Rank Predictions**: For each of the ranked experiment IDs individually:
  - Spawn a subagent instructed to invoke the `rank-predictions` skill, passing to it the list of prediction IDs from the previous step, and a single experiment ID. This subagent will return a ranked list of theory IDs for that one experiment. It might report some theory IDs as NO_PREDICTION if those theories do not make predictions for that experiment (that is fine). All subagents can run in parallel.
5. **Score Prediction Rankings**: For each theory ID individually, invoke a script to calculate the prediction accuracy score and the prediction coverage score:
  ```bash
  ...
  - <THEORY_ID> is the specific theory ID for which you are calculating the scores.
...
6. **Rank Explanatory Power**: Spawn a single subagent instructed to invoke the `rank-explanatory-power` skill, passing to it the list of theory IDs. It will return an explanatory power score for each theory. This subagent can run in parallel with the other subagents from the next step.
7. **Obtain Additional Per-Theory Scores**: For each of the theory IDs individually:
  - Spawn a subagent instructed to invoke the `score-theory-local-subscores` skill, passing to it the specific theory ID. The subagent will return multiple scores between 0 and 1: a soundness score, a length score, and a guidance adherence score. All subagents can run in parallel.
8. **Collect Subscores**: Wait for all subagents from the previous steps to finish and collect their final result messages.
9. **Overall Theory Score**: Combine the scores for each theory into an overall score object, one for each theory, by running:
  ```bash
...
10. **Save Scores**: Save the overall scores and the detailed subscores to a database, using this bash command:
  ```bash
...
11. **Final Output**: Report the list of all theory IDs along with their final scores. Also include a breakdown of the different subscores for each theory.
