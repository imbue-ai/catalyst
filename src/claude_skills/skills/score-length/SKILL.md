...
Besides the theory to score, you may optionally be given a STANDARD_LENGTH value. The default STANDARD_LENGTH, if not specified, is 4000 words.
...
2. **Score Length**: Calculate a length score for the theory using the formula `Length Score = min(1, 1 / (words_in_main_body / STANDARD_LENGTH)**2)`, where `words_in_main_body` is the number of words in the main body of the theory. Use ad-hoc Python code to perform this calculation, as described above.