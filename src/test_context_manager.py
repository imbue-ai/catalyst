...
class TestContextManager(unittest.TestCase):
    ...
    def test_store_results_all_agents(self):
        ...
        res = self.run_cmd("store_results", "--from_agent_type", "write-theory", "--from_folder", str(parent_dir))
        parent_theory_id = res.stdout.strip()
        ...
        for agent_type, category in AGENT_TYPE_MAP.items():
            ...
            args = ["store_results", "--from_agent_type", agent_type, "--from_folder", str(src_dir)]
    ...
    def test_create_context_all_agent_types(self):
        ...
        def store_simple(agent, cat, content_file, parent=None, extra_files=None):
            ...
            if extra_files:
                for f, c in extra_files.items():
                    (d / f).write_text(c)
            args = ["store_results", "--from_agent_type", agent, "--from_folder", str(d)]
            if parent:
                args += ["--parent_theory", parent]
            return self.run_cmd(*args).stdout.strip()
        ...
        x_id = store_simple("run-experiment", "experiment", "description.md", parent=t_id, extra_files={"script.py": "print(1)"})
        p_id = store_simple("predict-experiments", "prediction", "predictions.md", parent=t_id)
        ...
        target_agents = [
            ...
            ("rank-predictions", {"--from_experiment": x_id, "--from_prediction": p_id}),
        ...
        for agent, flags in target_agents:
            ...
            args = ["create_context", "--for_agent_type", agent, "--target_folder", str(target_folder)]
    ...
    def test_transactions(self):
        ...
        res = self.run_cmd("store_results", "--from_agent_type", "write-theory", "--from_folder", str(src_dir), tx_id=tx_id)
        ...
        entries = json.loads(res.stdout)
        self.assertEqual(len(entries), 1, f"Expected 1 entry with tx_id={tx_id}, got 0.")
    ...
    def test_search_experiments(self):
        ...
        self.run_cmd("store_results", "--from_agent_type", "run-experiment", "--from_folder", str(src_dir), "--metadata", "tags=quantum,gravity,test")
    ...
    def test_rescore_and_sample(self):
        ...
        def store_t(content):
            ...
            return self.run_cmd("store_results", "--from_agent_type", "write-theory", "--from_folder", str(d)).stdout.strip()

        def store_r(t_id):
            d = self.test_dir / f"r_{t_id}"
            d.mkdir(parents=True, exist_ok=True)
            (d / "review.md").write_text("Review content")
            return self.run_cmd("store_results", "--from_agent_type", "falsify-hypothesis", "--from_folder", str(d), "--parent_theory", t_id).stdout.strip()
    ...
    def test_fetch_helpers(self):
        ...
        res = self.run_cmd("store_results", "--from_agent_type", "literature-review", "--from_folder", str(lit_dir))
        l_id = res.stdout.strip()
        ...
        res = self.run_cmd("store_results", "--from_agent_type", "run-experiment", "--from_folder", str(exp_dir))
        x_id = res.stdout.strip()
        
        target = self.test_dir / "fetch_target"
        target.mkdir()
        
        self.run_cmd("fetch_literature", "--target_folder", str(target), "--from_literature", l_id)
        self.assertTrue((target / "literature" / l_id / "summary.md").exists())
        
        self.run_cmd("fetch_experiment", "--target_folder", str(target), "--from_experiment", x_id)
    ...
    def test_export_population(self):
        ...
        self.run_cmd("store_results", "--from_agent_type", "write-theory", "--from_folder", str(src_dir))
